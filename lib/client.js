  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      async function fetchLatestTag() {
        try {
          ...
          if (res.ok) {
            ...
            const stable = (Array.isArray(tags) ? tags : []).map(t => t.name).filter(n => typeof n === 'string' && /^v\d+\.\d+\.\d+$/.test(n));
            if (stable.length > 0) return stable.reduce((a, b) => (semverCmp(b, a) > 0 ? b : a));
          }
        } catch (_) { /* fall through */ }
        try {
          const res2 = await fetch('https://raw.githubusercontent.com/' + MIRROR + '/main/package.json');
          if (res2.ok) { const j = await res2.json(); return /^\d+\.\d+\.\d+$/.test(j.version) ? ('v' + j.version) : undefined; }
        } catch (_) { /* ignore */ }
        return undefined;
      }
      function promptText(tag) {
        return '帮我更新 dsh-paste-input 插件到 ' + tag + '\n1. 执行 dsh plugin --profile web add \'' + PKG + '@github:' + MIRROR + '#' + tag + '\'（首次可能被 pnpm 11 拦截构建脚本，则先在 ~/.dsh/profiles/web 执行 pnpm approve-builds --all）\n2. 完成后提醒我硬刷新浏览器（Ctrl/Cmd+Shift+R）';
      }
      async function startUpdateChip() {
        const tag = await fetchLatestTag();
        if (!tag) return;
        if (semverCmp(tag, PLUGIN_VERSION) <= 0) return;
        ...
        let bottom = 12;
        for (const ch of chips) { const rr = ch.getBoundingClientRect(); bottom = Math.max(bottom, window.innerHeight - rr.top + 8); }
        el.style.cssText = 'position:fixed;left:12px;bottom:' + bottom + 'px;z-index:2147483000;display:inline-flex;align-items:center;gap:8px;padding:6px 10px;border:1px solid #4a7dff;border-radius:10px;background:#1e2430;color:#cfe0ff;font:12px/1.4 system-ui,Segoe UI,sans-serif;cursor:pointer;box-shadow:0 6px 18px rgba(0,0,0,.35);';
        ...
        cls.addEventListener('click', function (ev) { ev.stopPropagation(); el.remove(); });
      ...
      const remove = (sessionId, occurrence) => {
        ...
        // The occurrence owns its whole inline range ("@" + label), so the
        // removal must span occurrence.length, not one character; leaving
        // characters behind demotes the chip to plain draft text.
