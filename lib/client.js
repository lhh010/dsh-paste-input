window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = `
      ...
      .dshca-chat-attachments{display:flex;flex-wrap:wrap;gap:6px;padding:2px 16px 10px}
      ...
      .dshca-chat-usertext{white-space:pre-wrap;word-break:break-word;color:inherit;font:inherit}
    `;
    ...
    function foldAttachmentDiv(div) {
      ...
      const parsed = parseAttachmentBlock(text);
      if (parsed === null) {
      ...
      const userWords = text.slice(0, parsed.blockStart) + text.slice(parsed.blockEnd);
      if (userWords.trim() !== '') {
        const words = document.createElement('div');
        words.className = 'dshca-chat-usertext';
        words.textContent = userWords;
      ...
      for (const file of parsed.files) {
        ...
        const absolute = parsed.root + '/' + file.path;
        ...
        if (parsed.raw !== undefined) {
          ...
          if (parsed.manifest !== '') tipLines.push('清单：' + parsed.manifest);
