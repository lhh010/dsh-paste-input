  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      async function startUpdateChip() {
        ...
        el.style.cssText = 'position:fixed;left:12px;bottom:12px;z-index:2147483000;padding:6px 10px;border:1px solid #4a7dff;border-radius:10px;background:#1e2430;color:#cfe0ff;font:12px/1.4 system-ui,Segoe UI,sans-serif;cursor:pointer;box-shadow:0 6px 18px rgba(0,0,0,.35);';
        el.textContent = '⟳ paste-input 新版本 ' + tag + ' 可用，点击更新';
        el.addEventListener('click', () => {
          const txt = promptText(tag);
          try { navigator.clipboard?.writeText(txt); } catch (_) { /* clipboard unavailable */ }
          el.textContent = '已复制更新提示词到剪贴板，粘贴发送即可';
