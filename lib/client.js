window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // --- dsh-paste-input 自动版本检查 + 更新 chip（本地版本取自 Host /health，GitHub 仅查远端） ---
      const UPDATE_ID = 'dsh-paste-input';
      const PKG = '@dsh-community/dsh-paste-input';
      const MIRROR = 'lhh010/dsh-paste-input';
      // Local version: resolved live from the Host /health endpoint (the real
      // installed package version) with a frozen fallback for offline hosts.
      // The fallback MUST be bumped together with package.json on release —
      // sync-mirrors.mjs fails loud on drift.
      const PLUGIN_VERSION_FALLBACK = '0.1.28';
      ...
      async function resolveLocalVersion() {
        try {
          ...
          if (res.ok) {
            ...
            if (typeof j?.version === 'string' && /^\d+\.\d+\.\d+$/.test(j.version)) return j.version;
          }
        } catch (_) { /* 旧版 Host 无 version 字段或不可达——回退烙死常量 */ }
      ...
      async function startUpdateChip() {
        PLUGIN_VERSION = await resolveLocalVersion();
