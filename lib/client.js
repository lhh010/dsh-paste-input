window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // ── DSH-gated update check ────────────────────────────────────────
      // The host /latest route reports the running DSH version; the repo's
      // compatibility.json maps plugin tags to supported DSH versions. When
      // the newest tag does not support the running DSH, the chip explains
      // that instead of offering the update (falls back to the plain prompt
      // when either piece is unavailable).
      async function fetchDshVersion() {
        try {
          const res = await fetch('/dsh-paste-input/v1/latest', { signal: AbortSignal.timeout(6000) });
          if (!res.ok) return undefined;
          const b = await res.json();
          return typeof b.dshVersion === 'string' && b.dshVersion !== '' ? b.dshVersion : undefined;
        } catch (_) { return undefined; }
      }
      async function fetchCompatMap() {
        try {
          const res = await fetch('https://raw.githubusercontent.com/' + MIRROR + '/main/compatibility.json', { signal: AbortSignal.timeout(8000) });
          if (!res.ok) return undefined;
          const j = await res.json();
          if (j === null || typeof j !== 'object' || j.schema !== 1 || j.versions === null || typeof j.versions !== 'object') return undefined;
          const map = {};
          for (const tag of Object.keys(j.versions)) {
            if (!/^v\d+\.\d+\.\d+$/.test(tag)) continue;
            const dsh = j.versions[tag] && j.versions[tag].dsh;
            if (Array.isArray(dsh) && dsh.every(v => typeof v === 'string' && v !== '')) map[tag] = dsh;
          }
          return Object.keys(map).length > 0 ? map : undefined;
        } catch (_) { return undefined; }
      }
      function decideGatedUpdate(local, latest, dshVersion, compatMap) {
        if (semverCmp(latest, local) <= 0) return { kind: 'current' };
        if (dshVersion === undefined || compatMap === undefined) return { kind: 'available' };
        let supported;
        for (const tag of Object.keys(compatMap)) {
          if (!compatMap[tag].includes(dshVersion)) continue;
          if (supported === undefined || semverCmp(tag, supported) > 0) supported = tag;
        }
        if (supported !== undefined && semverCmp(supported, latest) > 0) supported = latest;
        if (supported === undefined || semverCmp(supported, local) <= 0) {
          return { kind: 'blocked', tag: latest, dshVersion: dshVersion };
        }
        if (semverCmp(supported, latest) >= 0) return { kind: 'available' };
        return { kind: 'partial', tag: supported, blocked: latest, dshVersion: dshVersion };
      }
      ...
      async function startUpdateChip() {
        ...
        const [dshVersion, compatMap] = await Promise.all([fetchDshVersion(), fetchCompatMap()]);
        const notice = decideGatedUpdate('v' + PLUGIN_VERSION, tag, dshVersion, compatMap);
        if (notice.kind === 'partial') { renderChip(notice.tag, notice); return; }
        if (notice.kind === 'blocked') { renderBlockedChip(notice.tag, notice.dshVersion); return; }
        renderChip(tag);
      }
      function renderChip(tag, partial) {
        ...
        if (partial) el.setAttribute('title', '更新到 ' + tag + '（另有 ' + partial.blocked + ' 需要更高 DSH 版本，当前 DSH ' + partial.dshVersion + ' 不支持）');
        ...
        lbl.textContent = partial === undefined
          ? '⟳ paste-input 新版本 ' + tag + ' 可用，点击更新'
          : '⟳ paste-input 新版本 ' + tag + ' 可用（' + partial.blocked + ' 需更高 DSH）';
      ...
      function renderBlockedChip(tag, dshVersion) {
        ...
        el.style.cssText = 'position:fixed;left:12px;z-index:2147483000;display:inline-flex;align-items:center;gap:8px;padding:6px 10px;border:1px solid #8a6d2f;border-radius:10px;background:#2d2718;color:#e8cf9a;font:12px/1.4 system-ui,Segoe UI,sans-serif;box-shadow:0 4px 12px rgba(0,0,0,.3);';
        const lbl = document.createElement('span');
        lbl.style.cssText = 'pointer-events:none;';
        lbl.textContent = '⧗ paste-input 新版本 ' + tag + ' 支持更高 DSH 版本，当前 DSH ' + dshVersion + ' 暂不可用';
        const cls = document.createElement('button');
        cls.textContent = '×';
        cls.setAttribute('aria-label', '关闭');
        cls.style.cssText = 'pointer-events:auto;border:0;background:transparent;color:#b7a677;font:inherit;cursor:pointer;padding:0 2px;line-height:1;';
        cls.addEventListener('click', function (ev) { ev.stopPropagation(); el.remove(); relayoutUpdateChips(); });
        el.appendChild(lbl); el.appendChild(cls);
      ...
      function renderOfflineChip() {
        ...
        var retryOnce = function () {
          ...
          fetchLatestTag(true).then(function (t) {
            ...
            if (semverCmp(t, PLUGIN_VERSION) > 0) {
              Promise.all([fetchDshVersion(), fetchCompatMap()]).then(function (gate) {
                const notice = decideGatedUpdate('v' + PLUGIN_VERSION, t, gate[0], gate[1]);
                if (notice.kind === 'partial') { renderChip(notice.tag, notice); return; }
                if (notice.kind === 'blocked') { renderBlockedChip(notice.tag, notice.dshVersion); return; }
                renderChip(t);
              });
