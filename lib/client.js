  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      async function latestFromTags() {
        try {
          const res = await fetch('https://api.github.com/repos/' + MIRROR + '/tags?per_page=10', { signal: AbortSignal.timeout(8000) });
          if (res.ok) {
      ...
      async function fetchLatestTag() {
        ...
        var r = await Promise.all([latestFromTags(), latestFromRaw()]);
        return r[0] || r[1];
      }
      ...
      function renderOfflineChip() {
        ...
        el.setAttribute('data-update-chip', UPDATE_ID);
        el.setAttribute('title', '无法连接 GitHub 查询新版本（可能是网络不可达）');
        ...
        lbl.style.cssText = 'pointer-events:none;';
        lbl.textContent = '⚠ paste-input 版本检查失败（网络不可达），点击重试';
        ...
        var retryOnce = function () {
          ...
          fetchLatestTag().then(function (t) {
            retrying = false;
