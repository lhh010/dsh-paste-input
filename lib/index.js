...
function detectDshVersion() {
  ...
  try {
    ...
    if (typeof argv1 === 'string' && argv1 !== '') {
      ...
      if (manifest.name === '@deepseek-ai/dsh' && typeof manifest.version === 'string' && manifest.version !== '') {
        dshVersionValue = manifest.version;
      }
    }
  } catch (_) { /* launched outside the dsh package layout */ }
...
// Preview endpoint: image files only (SVG excluded — served same-origin it
// would execute scripts), mapped from extension to a safe content type.
...
export function apply(ctx, config = {}) {
  ...
  const route = async (req, res) => {
    ...
    try {
      ...
      // Read-only: the running DSH version, for the client-side gated update
      // check (decides whether the newest plugin tag supports this host).
      if (req.method === 'GET' && suffix === '/latest') {
        json(res, 200, { ok: true, plugin: name, dshVersion: detectDshVersion() ?? null });
        return;
      }

      // Read-only image preview: serves one DECLARED image file out of a
      // committed send directory. The send must carry our ownership marker
      // (proves it is a dsh-paste-input upload, and lists its files), the
      // requested path must be one of the marker's declared actualPaths,
      // and resolution must stay inside the send root. Images only.
      if (req.method === 'GET' && suffix === '/file') {
        const rootParam = url.searchParams.get('root') ?? '';
        const pathParam = url.searchParams.get('path') ?? '';
        if (rootParam === '' || pathParam === '') throw new Error('root and path are required');
        const mimeType = PREVIEW_TYPES.get(extname(pathParam).toLowerCase());
        if (mimeType === undefined) throw new Error('preview endpoint serves image files only (png/jpeg/gif/webp/bmp/avif/ico)');
        const rel = pathParam.replaceAll('\\', '/');
        if (rel.split('/').some(part => part === '' || part === '.' || part === '..')) {
          throw new Error('attachment path contains an unsafe segment');
        }
        const root = resolve(rootParam);
        const marker = JSON.parse(await readFile(join(root, OWNER_FILE), 'utf8'));
        if (marker?.owner !== name || marker?.version !== 1) {
          throw new Error('not a dsh-paste-input send directory');
        }
        const declared = Array.isArray(marker.files) && marker.files.some(file => file?.actualPath === rel);
        if (!declared) throw new Error(`file ${rel} is not part of this send`);
        const target = resolve(root, rel);
        ensureInside(root, target);
        const info = await stat(target);
        if (!info.isFile()) throw new Error(`${rel} is not a regular file`);
        if (info.size > MAX_PREVIEW_BYTES) throw new Error(`${rel} exceeds the preview size cap`);
        res.writeHead(200, {
