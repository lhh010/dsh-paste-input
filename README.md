...
dsh plugin --profile web add '@dsh-community/dsh-paste-input@github:dsh-external/dsh-paste-input#v0.1.28'
...
声明支持 dsh-v0.1.6-alpha.1（npm 已发布，钉版本实机验证；client 插件面零代码差异，lib node --check 全绿，实机加载正常）。安装命令统一更新为 `#v0.1.28`。
...
> 1. 执行 `dsh plugin --profile web add '@dsh-community/dsh-paste-input@github:dsh-external/dsh-paste-input#v0.1.28'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
