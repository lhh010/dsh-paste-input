...
> - DSH **0.1.1-rc.2**（npm 最新）：装**旧版** `'@dsh-external/dsh-paste-input@github:omdsh-dev/dsh-paste-input#v0.1.5'`
> - DSH **0.1.2-alpha.1 / alpha.2**：装**新版**（下方默认命令）
## 安装（profile 模式）

```sh
# 方式一：git 依赖固定 tag（公开镜像，推荐；也可用 github:omdsh-dev/dsh-paste-input）
...
> 1. 执行 `dsh plugin --profile web add '@dsh-community/dsh-paste-input@github:omdsh-dev/dsh-paste-input#v0.1.8'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
> 遇到报错先查 https://github.com/omdsh-dev/dsh-paste-input README 的常见问题/已知限制。
